        ...
        <div class="content-wrapper">
            <div class="row">
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Editing User Info</h4>
                            <form method="POST" action="{{route('admin.user.update',['id'=>$user->id])}}" enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <label for="username">Username</label>
                                    <input type="text" class="form-control" id="name" name="username" placeholder="username" value="{{$user->username}}" >
                                    @error('username')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="first_name" name="first_name" placeholder="Firstname" value="{{$user->first_name}}" >
                                    @error('first_name')
                                    <span class="text-danger" style="color: red">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="phone_number" name="phone_number" placeholder="Phone number"
                                           oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" value="{{$user->phone_number}}" >
                                    @error('phone_number')
                                    <span class="text-danger" style="color: red">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="checkbox" id="isAdmin" name="isAdmin" @if($user->isAdmin == 1) checked @endif>
                                </div>
